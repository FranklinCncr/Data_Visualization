...
function Manhatan(matrix1,matrix2){
  let sum=0;  
  for(let i=0;i<12;i++){
    for(let j=0;j<77;j++){
...
function Euclidean(matrix1,matrix2){
  let sum=0;  
  for(let i=0;i<12;i++){
    for(let j=0;j<77;j++){
      if(matrix1[i][j]!=0 && matrix2[i][j]!=0){
  ...
  let result=Number(Math.sqrt(sum).toFixed(6));
  return result;
}
function Pearson(matrix1,matrix2){
  let xy=0;
  let sx=0;
  let sy=0;
  let sx2=0;
  ...
  for(let i=0;i<12;i++){
    for(let j=0;j<77;j++){
      if(matrix1[i][j]!=0 && matrix2[i][j]!=0){
        xy+=matrix1[i][j]*matrix2[i][j];
        sx+=matrix1[i][j];
        sy+=matrix2[i][j];
        sx2+=Math.pow(matrix1[i][j],2);
        sy2+=Math.pow(matrix2[i][j],2);
        n=n+1;
      }
    }
  }
  let result = (xy-((sx*sy)/n))/(Math.sqrt(sx2-(sx*sx/n))*Math.sqrt(sy2-(sy*sy/n)))     
  result=Number(result.toFixed(6));
  return result;
}
function Coseno(matrix1,matrix2){
  let xy=0;
  let lxl=0;
  let lyl=0;    
  for(let i=0;i<12;i++){
    for(let j=0;j<77;j++){
      if(matrix1[i][j]!=0 && matrix2[i][j]!=0){
        xy+=matrix1[i][j]*matrix2[i][j];
        lxl+=Math.pow(matrix1[i][j],2);
        lyl+=Math.pow(matrix2[i][j],2);
      }
    }    
  }
  let result = xy/(Math.sqrt(lxl)*Math.sqrt(lyl))     
  result=Number(result.toFixed(6));
  return result;
}
...
d3.selectAll(".button_click").on("click", async function () {
  ...
  if (buttonId == "addMap") {
    ...
    const geojson = topojson.feature(
      data,
      data.objects["Boundaries - ZIP Codes"]
    );
    ...
    const stateCapitalElements = svg.selectAll("g").data(dataSample).join("g");
    ...
    for (let i = 0; i < dataCrimes.length; i++) {
      ...
      let info =
        "Type: " +
        dataCrimes[i]["Primary Type"] +
        "\n" +
        "Description: " +
        dataCrimes[i]["Description"] +
        "\n" +
        "Location: " +
        dataCrimes[i]["Location Description"] +
        "\n" +
        "Comunity Area: " +
        namesAreas[dataCrimes[i]["CommunityArea"] - 1];
      if (dataCrimes[i].IUCR == iucr1.value) {
        if (
          dataCrimes[i].CommunityArea == area1.value ||
          dataCrimes[i].CommunityArea == area2.value ||
          dataCrimes[i].CommunityArea == area3.value ||
          dataCrimes[i].CommunityArea == area4.value ||
          dataCrimes[i].CommunityArea == area5.value ||
          area1.value == "all" ||
          area2.value == "all" ||
          area3.value == "all" ||
          area4.value == "all" ||
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      for (let j = 0; j < dataCrimes.length; j++) {
        if (iucr1.value == dataCrimes[j].IUCR) {
          if (
            area1.value == dataCrimes[j].CommunityArea ||
            area2.value == dataCrimes[j].CommunityArea ||
            area3.value == dataCrimes[j].CommunityArea ||
            area4.value == dataCrimes[j].CommunityArea ||
            area5.value == dataCrimes[j].CommunityArea ||
            area1.value == "all" ||
            area2.value == "all" ||
            area3.value == "all" ||
            area4.value == "all" ||
  ...
  if (buttonId == "d3chartE1") {
    ...
    if (
      ...
      let countAreasT1 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
      let countAreasT2 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
      let countAreasT3 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
      let countAreasT4 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
      let countAreasT5 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
  ...
  /*****************************************Compare 2 chart 3d crime***************************************************** */
  if (buttonId == "d3chartCrime1") {
    var iucr1 = d3.select("#iucr1").node();
    var iucr2 = d3.select("#iucr2").node();
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      dataTotal.push(countAreasT1);
      countAreasT1 = Array.apply(null, Array(77)).map(
        Number.prototype.valueOf,
        0
      );
    }
    ...
    var layout = {
      title: iucr1.options[iucr1.selectedIndex].text,
      scene: {
        xaxis: { title: "X" },
        yaxis: { title: "Y" },
        zaxis: { title: "Z" },
    ...
    Plotly.newPlot("chart3dCrime1", data, layout);
  }
  /****************************************************char 2*********************************************** */
  if (buttonId == "d3chartCrime2") {
    var iucr1 = d3.select("#iucr2").node();
    ...
    Plotly.newPlot("chart3dCrime2", data, layout);
  }
  /*****************************************************KNN Implementation **********************************************************************/
  if (buttonId == "knn") {
    
    var knn = d3.select("#knn-input").node();
    var n = d3.select("#numberknn").node();
    ...
    let iucr=['1320','486','5002','820','281','1153','1540','1753','1330','1544','3960','320','143A','110','470','560','610','910','1020','1506','2024'];
    let nameIucr=["Criminal damage","Battery","Other Offense","Thef","Crim sexual assault","Deceptipe practice","Obscenity","Offense involving children","Criminal Trespass","Sex ofense","Intimidation","Robbery","Weapons Violation","Homicide","Public Peace Violation","Assault","Bruglary","Motor Vehicle Thef","Arson","Prostitution","Narcotics"]
    let nAreasTimes = Array(21).fill().map(()=> Array(12).fill().map(() => Array(77).fill(0)));
    //[area][mont]
    for(let i=0;i<months.length;i++){      
      const dataCrimes = await d3.csv("../data/"+months[i]+".csv");
      for(let j=0;j<dataCrimes.length;j++){
        for(let k=0;k<iucr.length;k++){
          if(iucr[k]==dataCrimes[j].IUCR){
            switch (parseInt(dataCrimes[j].CommunityArea)) {
              case 1:nAreasTimes[k][i][0]++;break;
            case 2:nAreasTimes[k][i][1]++;break;
            case 3:nAreasTimes[k][i][2]++;break;
            case 4:nAreasTimes[k][i][3]++;break;
            case 5:nAreasTimes[k][i][4]++;break;
            case 6:nAreasTimes[k][i][5]++;break;
            case 7:nAreasTimes[k][i][6]++;break;
            case 8:nAreasTimes[k][i][7]++;break;
            case 9:nAreasTimes[k][i][8]++;break;
            case 10:nAreasTimes[k][i][9]++;break;
            case 11:nAreasTimes[k][i][10]++;break;
            case 12:nAreasTimes[k][i][11]++;break;
            case 13:nAreasTimes[k][i][12]++;break;
            case 14:nAreasTimes[k][i][13]++;break;
            case 15:nAreasTimes[k][i][14]++;break;
            case 16:nAreasTimes[k][i][15]++;break;
            case 17:nAreasTimes[k][i][16]++;break;
            case 18:nAreasTimes[k][i][17]++;break;
            case 19:nAreasTimes[k][i][18]++;break;
            case 20:nAreasTimes[k][i][19]++;break;
            case 21:nAreasTimes[k][i][20]++;break;
            case 22:nAreasTimes[k][i][21]++;break;
            case 23:nAreasTimes[k][i][22]++;break;
            case 24:nAreasTimes[k][i][23]++;break;
            case 25:nAreasTimes[k][i][24]++;break;
            case 26:nAreasTimes[k][i][25]++;break;
            case 27:nAreasTimes[k][i][26]++;break;
            case 28:nAreasTimes[k][i][27]++;break;
            case 29:nAreasTimes[k][i][28]++;break;
            case 30:nAreasTimes[k][i][29]++;break;
            case 31:nAreasTimes[k][i][30]++;break;
            case 32:nAreasTimes[k][i][31]++;break;
            case 33:nAreasTimes[k][i][32]++;break;
            case 34:nAreasTimes[k][i][33]++;break;
            case 35:nAreasTimes[k][i][34]++;break;
            case 36:nAreasTimes[k][i][35]++;break;
            case 37:nAreasTimes[k][i][36]++;break;
            case 38:nAreasTimes[k][i][37]++;break;
            case 39:nAreasTimes[k][i][38]++;break;
            case 40:nAreasTimes[k][i][39]++;break;
            case 41:nAreasTimes[k][i][40]++;break;
            case 42:nAreasTimes[k][i][41]++;break;
            case 43:nAreasTimes[k][i][42]++;break;
            case 44:nAreasTimes[k][i][43]++;break;
            case 45:nAreasTimes[k][i][44]++;break;
            case 46:nAreasTimes[k][i][45]++;break;
            case 47:nAreasTimes[k][i][46]++;break;
            case 48:nAreasTimes[k][i][47]++;break;
            case 49:nAreasTimes[k][i][48]++;break;
            case 50:nAreasTimes[k][i][49]++;break;
            case 51:nAreasTimes[k][i][50]++;break;
            case 52:nAreasTimes[k][i][51]++;break;
            case 53:nAreasTimes[k][i][52]++;break;
            case 54:nAreasTimes[k][i][53]++;break;
            case 55:nAreasTimes[k][i][54]++;break;
            case 56:nAreasTimes[k][i][55]++;break;
            case 57:nAreasTimes[k][i][56]++;break;
            case 58:nAreasTimes[k][i][57]++;break;
            case 59:nAreasTimes[k][i][58]++;break;
            case 60:nAreasTimes[k][i][59]++;break;
            case 61:nAreasTimes[k][i][60]++;break;
            case 62:nAreasTimes[k][i][61]++;break;
            case 63:nAreasTimes[k][i][62]++;break;
            case 64:nAreasTimes[k][i][63]++;break;
            case 65:nAreasTimes[k][i][64]++;break;
            case 66:nAreasTimes[k][i][65]++;break;
            case 67:nAreasTimes[k][i][66]++;break;
            case 68:nAreasTimes[k][i][67]++;break;
            case 69:nAreasTimes[k][i][68]++;break;
            case 70:nAreasTimes[k][i][69]++;break;
            case 71:nAreasTimes[k][i][70]++;break;
            case 72:nAreasTimes[k][i][71]++;break;
            case 73:nAreasTimes[k][i][72]++;break;
            case 74:nAreasTimes[k][i][73]++;break;
            case 75:nAreasTimes[k][i][74]++;break;
            case 76:nAreasTimes[k][i][75]++;break;
            case 77:nAreasTimes[k][i][76]++;break;
            default:
              break;
            }
          }
    ...
    let results=Array.apply(null, Array(21)).map(Number.prototype.valueOf,0);    
    console.log(nameIucr[knn.value]);

    for(let i=0;i<21;i++){
      //results[i]=Manhatan(nAreasTimes[knn.value],nAreasTimes[i]);
      results[i]=Euclidean(nAreasTimes[knn.value],nAreasTimes[i]);
      //results[i]=Pearson(nAreasTimes[knn.value],nAreasTimes[i]);
      //results[i]=Coceno(nAreasTimes[knn.value],nAreasTimes[i]);     
